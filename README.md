...
| Async worker | SQS (ElasticMQ locally); `service`-role workers — outbox relay, image worker (S3 ObjectCreated), catalog cache-invalidation consumer, cart product-event consumer, reservation reaper |
...
make compose-up               # Postgres + Valkey + LocalStack (S3/SNS/SQS) + ElasticMQ + relay + image-worker + cache-worker + cart-consumer + reaper
