...
async def test_cold_stampede_single_fills_through_the_real_valkey_adapter(real_valkey):
    ...
    assert await real_valkey.get(f"product:{pid}") is not None


async def test_real_invalidation_clears_value_and_in_flight_fill_lock(real_valkey):
    ...
    result, _ = await asyncio.gather(read(), invalidate_mid_fill())
    assert result is not None  # the caller still gets its answer
    assert await real_valkey.get(f"product:{pid}") is None  # the stale fill never landed
    assert repo.get_calls == 1
