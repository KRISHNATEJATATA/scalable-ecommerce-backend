...
Returns a keyset **page** of every attempt for an order, newest first by default
(``PageParams`` default ``-created_at``; retries share ``order_id`` and only
``idempotency_key`` is unique) — reconciliation decides which is authoritative.

Ticket 2 sketched a single-result ``get_by_order_id``; that contract can't
represent retries, so the method is named ``list_by_order_id`` and returns a page.
