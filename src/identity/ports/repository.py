...
class IdentityRepositoryPort(Protocol):
    async def lock(self, oidc_sub: str) -> None:
        """Advisory-lock the account for the current transaction (released on commit)."""
        ...
    ...
    async def get_or_create(
        self, oidc_sub: str, email: str, outbox: OutboxFactory | None = None, *, is_active: bool = True
    ) -> Any: ...
